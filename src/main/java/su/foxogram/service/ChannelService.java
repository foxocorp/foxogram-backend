...
public class ChannelService {
	...
	public ChannelService(ChannelRepository channelRepository, MemberService memberService, WebSocketService webSocketService, AttachmentService attachmentService) {
	...
	public Channel update(Member member, Channel channel, ChannelEditDTO body) throws Exception {
		...
		webSocketService.sendMessageToSessions(getRecipients(channel), GatewayConstant.Opcode.DISPATCH.ordinal(), new ChannelDTO(channel, null), GatewayConstant.Event.CHANNEL_UPDATE.getValue());
	...
	public void delete(Channel channel, User user) throws Exception {
		...
		webSocketService.sendMessageToSessions(getRecipients(channel), GatewayConstant.Opcode.DISPATCH.ordinal(), Map.of("id", channel.getId()), GatewayConstant.Event.CHANNEL_DELETE.getValue());
	...
	public Member addMember(Channel channel, User user) throws Exception {
		...
		webSocketService.sendMessageToSessions(getRecipients(channel), GatewayConstant.Opcode.DISPATCH.ordinal(), new MemberDTO(member, true), GatewayConstant.Event.MEMBER_ADD.getValue());
	...
	public void removeMember(Channel channel, User user) throws Exception {
		...
		webSocketService.sendMessageToSessions(getRecipients(channel), GatewayConstant.Opcode.DISPATCH.ordinal(), new MemberDTO(member, true), GatewayConstant.Event.MEMBER_REMOVE.getValue());
