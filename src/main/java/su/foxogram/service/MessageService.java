...
public class MessageService {
	...
	public MessageService(MessageRepository messageRepository, WebSocketService webSocketService, ChannelService channelService, MemberService memberService, AttachmentService attachmentService) {
	...
	public Message add(Channel channel, User user, MessageCreateDTO body) throws Exception {
		...
		webSocketService.sendMessageToSessions(getRecipients(channel), GatewayConstant.Opcode.DISPATCH.ordinal(), new MessageDTO(message, true), GatewayConstant.Event.MESSAGE_CREATE.getValue());
	...
	public void delete(long id, Member member, Channel channel) throws Exception {
		...
		webSocketService.sendMessageToSessions(getRecipients(channel), GatewayConstant.Opcode.DISPATCH.ordinal(), Map.of("id", id), GatewayConstant.Event.MESSAGE_DELETE.getValue());
	...
	public Message update(long id, Channel channel, Member member, MessageCreateDTO body) throws Exception {
		...
		webSocketService.sendMessageToSessions(getRecipients(channel), GatewayConstant.Opcode.DISPATCH.ordinal(), new MessageDTO(message, true), GatewayConstant.Event.MESSAGE_UPDATE.getValue());
